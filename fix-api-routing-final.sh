...
# 🔧 Fix API Routing - Final Solution
# This script fixes the API routing by properly handling the /api/ prefix

set -e

echo "🔧 Fixing API Routing - Final Solution"
echo "======================================"

# Check current Nginx configuration
echo "📊 Step 1: Checking current Nginx configuration..."
echo "Current /api/ location block:"
grep -A 15 "location /api/" /etc/nginx/sites-available/bmi.engage-360.net
...
server {
    ...
    # Backend API - FIXED: Properly handle /api/ prefix
    location /api/ {
        # Proxy to backend without removing /api/ prefix
...
echo ""
echo "📊 Step 6: Testing backend directly for comparison..."
echo "Testing backend API stats:"
curl -f http://localhost:3006/api/search/stats && echo "✅ Backend API stats OK" || echo "❌ Backend API stats FAILED"

echo ""
echo "Testing backend API documents:"
curl -f "http://localhost:3006/api/documents?skip=0&limit=10" && echo "✅ Backend API documents OK" || echo "❌ Backend API documents FAILED"

# Test the main site
echo ""
echo "📊 Step 7: Testing main site..."
...
echo ""
echo "🎉 API routing should now be fixed!"
echo ""
echo "Your BMI Chat application should now work correctly at:"
echo "  https://bmi.engage-360.net"
echo ""
echo "The key fix was:"
echo "  - Changed proxy_pass from 'http://localhost:3006/' to 'http://localhost:3006'"
echo "  - This preserves the /api/ prefix that the backend expects"
echo ""
...
echo "3. Test backend directly: curl http://localhost:3006/api/search/stats" 